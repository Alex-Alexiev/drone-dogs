...
class CommNode(Node):
    def __init__(self):
        ...
        self.srv_land = self.create_service(Trigger, 'rob498_drone_7/comm/land', self.callback_land)
    ...
    def callback_land(self, request, response):
        self.handle_land()
        response.success = True
        response.message = "Land command executed: Landing."
        return response
    ...
    def handle_land(self):
        self.get_logger().info("Land command received. Setting desired pose to initial pose.")
        self.get_logger().info(f"Initial pose: {self.initial_pose.pose.position}")
